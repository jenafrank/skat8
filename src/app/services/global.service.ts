...
export class GlobalService {
  ...
  getFilteredRoundPlayers(roundPlayers?: string[]): string[] {
    ...
    const filteredRoundPlayers: string[] = [];
    for (const ply of roundPlayers) {
      if (ply !== '😶') {
  ...
  incmod(spieltagData: any): number {
    const nrPly: number = this.filteredRoundPlayers.length;
    const maxGameNr: number = this.dataService.totalgame(spieltagData);
    const gamedata: GameDataRaw = spieltagData[this.dataService.game(maxGameNr)];
  ...
  calcActiveThree(spieltagData: any): string[] {
    ...
    const mod: number = this.incmod(spieltagData);

    if (nrPly === 4) {
      const p1: string = this.filteredRoundPlayers[0];
      const p2: string = this.filteredRoundPlayers[1];
      const p3: string = this.filteredRoundPlayers[2];
      const p4: string = this.filteredRoundPlayers[3];
    ...
    if (nrPly === 5) {
      ...
      const p3: string = this.filteredRoundPlayers[2];
      const p4: string = this.filteredRoundPlayers[3];
      const p5: string = this.filteredRoundPlayers[4];
