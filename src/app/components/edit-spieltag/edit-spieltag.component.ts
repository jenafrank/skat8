...
import { AppGameRemoveComponent } from './app-game-remove';
...
export class EditSpieltagComponent implements OnInit {
  ...
  buildHeader(): void {
    const maxround: number = this.logic.maxRoundFromDayData(this.spieltagData);
  ...
  buildGameArray(runde: number): void {
    const total: number = this.dataService.totalgame(this.spieltagData);

    let games: GameView[] = [];

    let game: GameDataRaw;
    let view: GameView;
    const pnts: Map<string, number> = new Map();
    let currentPlayers = '';
    let currentMod = 0;

    let i = 0; // total game nr
    let j = 0; // round game nr
    ...
    while (i < total) {
      ...
      const allPlayersSignature: string = game.allPlayers;
      if (currentPlayers !== allPlayersSignature) {
        ...
        for (const ply of splittedPlayers) {
        ...
        const splittedActive = game.activeThree.split(' ');

        for (const ply of splittedActive) {
  ...
  getEmptyView(): GameView {
    const ret: GameView = {} as GameView;
  ...
  removeLastGame(): void {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        const data: any[] = this.getCurrentDataSource().filteredData;

        console.log(data);

        if (isUndefined(data) || data.length === 0) { return; }

        const nr: number = data[data.length - 1].nr;
